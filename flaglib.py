...
def getKey(value):
    ...
    if value<80 :
        ...
        key='u'
    elif abs(value-964)<50:
        key='d'
    elif abs(value-730)<80:
        key='l'
    elif abs(value-489)<50:
        key='r'
    elif abs(value-246)<80:
        key='m'    
    return key
...
class Character:
    ...
    def __init__(self, pixels, width, height, lcd, color):
        self.lcd = lcd
        self.width = width
        self.height = height
        self.color = color
    ...
    def move(self, dx=0, dy=0, max_x=None, max_y=None):
        ...
        self._plot(self.x, self.y, x1, y1)
    ...
    def _plot(self, x0, y0, x1, y1):
        ...
        w = self.width + dw   #這次要畫的寬度
        h = self.height + dh  #這次要畫的高度
...
#        print(x, y, x+w-1, y+h-1, w, h)  #########
        ...
        for i1 in range(h):
            ...
            for j1 in range(w):
                ...
                #print(i, j, end=', ')#######
                p = self.pixels[self.pixels_idx][i*self.width + j]
